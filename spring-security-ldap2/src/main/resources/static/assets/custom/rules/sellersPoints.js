var SellersPoints = function() {
	...
	return {
		init : function() {
			...
			if (isForm(form)) {
				validate(form);
				$("#points,#returnPoints,#agentProfit",form).TouchSpin({
		            min: 0,
		            max: 100,
		            step: 0.1,
		            decimals: 2,
				...
				$('#earnestMoney,#fullCost',form).TouchSpin({
					min: 0,
		            max: 999999999,
		           // stepinterval: 50,
		            boostat: 5,
		...
		edit:function(dmid){
			$.get('/rule/sellersPoints/findone',{id:dmid},function(data,state){
				if (state == "success") {
		...
		remove:function(id,name,token){
			var data={'id':id};
	    	data[name]=token;
	    	var opt={url:'/rule/sellersPoints/remove',type:'POST',data:data};
