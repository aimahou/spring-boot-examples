var AdvertisManage = function() {
	...
	return {
		...
		edit:function(dmid){
			$.get('/operate/advertisManage/findone',{id:dmid},function(data,state){
				if (state == "success") {
					Stencil.fullForm(data);
					Stencil.modal();
				}
			},'JSON');
		},
		remove:function(id,name,token){
			var data={'id':id};
	    	data[name]=token;
	    	var opt={url:'/operate/advertisManage/remove',type:'POST',data:data};
